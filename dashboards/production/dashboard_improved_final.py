...
with tabs[0]:
    ...
    with st.expander("💵 **Compensation Structure** (Custom Base/Variable per Role)", expanded=False):
        ...
        with pnl_cols[0]:
            ...
            total_commission = (actual_revenue * closer_comm_rate) + (actual_revenue * setter_comm_rate) + (actual_revenue * 0.05)
            cogs = total_monthly_base + total_commission
            
            # Gross Profit
            gross_profit = actual_revenue - cogs
            
            # OpEx (operational costs - need to get from config)
            monthly_opex = st.session_state.get('monthly_opex', 100000)  # Default if not set
            ...
            st.plotly_chart(fig_pnl, use_container_width=True, key="pnl_flow")
        
        with pnl_cols[1]:
            st.markdown("**💰 P&L Metrics**")
            
            # Key margins
            gross_margin = (gross_profit / actual_revenue * 100) if actual_revenue > 0 else 0
            ebitda_margin = (ebitda / actual_revenue * 100) if actual_revenue > 0 else 0
            net_margin = (net_retained / actual_revenue * 100) if actual_revenue > 0 else 0
            
            st.metric("Gross Margin", f"{gross_margin:.1f}%")
            st.metric("EBITDA Margin", f"{ebitda_margin:.1f}%", 
                     "🟢 Healthy" if ebitda_margin > 20 else "🟡 Fair" if ebitda_margin > 10 else "🔴 Low")
